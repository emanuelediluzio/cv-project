...
# Gestisci i percorsi 'NaN'
df[path_col] = df[path_col].fillna('missing')

# Converti i valori della colonna dei percorsi in stringhe, elimina caratteri non validi e normalizza i percorsi
df[path_col] = df[path_col].apply(lambda x: os.path.abspath(str(x).strip().replace(';', '').replace(' ', '_').replace('(', '').replace(')', '').replace(',', '_')) if x != 'missing' else 'missing')
...
# Funzione per trovare l'ultimo checkpoint
def find_latest_checkpoint(directory, prefix):
    checkpoints = glob.glob(os.path.join(directory, prefix))
    if checkpoints:
        return max(checkpoints, key=os.path.getctime)
    return None

# Cerca un checkpoint con prefisso "resnet_2"
checkpoint_path = find_latest_checkpoint("/work/cvcs2024/air_pollution_prediction/Checkpoints", "resnet_2*.pth")
if checkpoint_path is None:
    # Se non esiste, cerca il vecchio checkpoint per il pretraining
    checkpoint_path = find_latest_checkpoint("/work/cvcs2024/air_pollution_prediction/Checkpoints", "cp-fine-epoch-*.pth")

if checkpoint_path is None:
    # Se ancora non esiste, usa il checkpoint predefinito
...
if os.path.exists(checkpoint_path):
    ...
    start_epoch = checkpoint.get('epoch', 0) + 1  # Riprendi dall'epoca successiva, se disponibile
else:
...
def calculate_rmse(outputs, labels):
    ...
    return np.sqrt(np.mean((outputs - labels) ** 2))
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{num_epochs}") as pbar:
            for batch_idx, (inputs, labels) in enumerate(train_loader):
                try:
                    ...
                    print(f"Errore durante l'elaborazione del batch {batch_idx}: {e}")
                    continue
